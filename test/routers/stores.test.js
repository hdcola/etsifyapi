...
describe('GET /api/stores/:store_id', () => {
    ...
    it('should return 200 and the store details if the store exists', async () => {        
       ...
       stores.findOne.mockResolvedValue(mockStore);
        const response = await request(app).get(`/api/stores/${mockStore.store_id}`);
    ...
    it('should return 404 if the store does not exist', async () => {
        ...
        const response = await request(app).get(`/api/stores/${store_id_nonexistent}`);  
...
/*
    ...
    */
    ...
    it('should return 400 Bad Request when database query fails', async () => {
        const SequelizeConnectionError = new Sequelize.ConnectionError('Database connection failed');
        SequelizeConnectionError.name = 'SequelizeConnectionError';
        ...
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty('status', 'error'); 
        expect(response.body).toHaveProperty('message', 'Failed to fetch stores');
        expect(response.body).not.toHaveProperty('errors');
    });
